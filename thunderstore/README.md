...
- **ExtraSlots** — dedicated slots for helmet/chest/legs/cape, two utility slots (Megingjord +
  ...
  ExtraSlots. Buy an inventory row from Haldor and it just works: the new row is an ordinary bag
  row, and the extra slots slide down underneath it with everything still in them. `Tune it:` `[Slots] EquipmentSlots=true`, `UtilitySlots=2`, `FoodSlots=3`, `AmmoSlots=3`, `GenericSlots=2`, `QuickSlots=0`, `AutoEatFromFoodSlots=true`
